...
public class AnalizadorAccesosAServidor
{
     ...
     * 
     * @param Nombre del archivo de log con extension.log 
     */
    ...
    public int obtenerHoraMasAccesos(){
        ...
        if(logServidor.size() > 0){
            for(int i = 0; i < 24; i++){
                int horaNuevaGuardada = i;
                int horaNuevaQueTieneMasAccesos =  0;
                for(Acceso log : logServidor){
                    if(log.getHoraDeAcceso() == horaNuevaGuardada){
                        horaNuevaQueTieneMasAccesos++;
                    }
                }

                if(horaNuevaQueTieneMasAccesos > contadorDeHorasIguales){
                    contadorDeHorasIguales = horaNuevaQueTieneMasAccesos;
                    horaConMasAccesos = horaNuevaGuardada;
                }else if(horaNuevaQueTieneMasAccesos == contadorDeHorasIguales && horaConMasAccesos < horaNuevaGuardada){
                    horaConMasAccesos = horaNuevaGuardada;
                }
            }
            logServidor.clear();
